...
```html
xmlns:vm="clr-namespace:DeleteButtonAdd"
d:DataContext="{d:DesignInstance vm:MainWindowViewModel, IsDesignTimeCreatable=True}
```
- Remove the startup URI in the App.xaml and override the startup method:
```csharp
...
```
- Create a MainWindowViewModel class
- Clean the solution and rebuild to update the XAML view
- Add a folder called ViewModels with 2 classes: BaseView and RelayCommand
- BaseView implements INotifyPropertyChanged. This will let the Listbox to listen for changes in its ItemSource
```csharp
...
```
- Create a ICommand derived class
```csharp
using System;
...
```
- Bind the button Click command to a RelayCommand
```html
...
```csharp
    ...
    private void AggiungiMethod(object param)
        {
            TextSaluto = new ObservableCollection<string>(); //will be overwritten every time the button is pressed. Add this to the MainWindowViewModel constructor to keep all the values. 
            TextSaluto.Add("ciao");
        }
